<script>
	...
	import { Navbar, NavBrand, NavLi, NavUl, NavHamburger, Button, Input, Avatar, Dropdown, DropdownItem, DropdownHeader, DropdownDivider} from 'flowbite-svelte'
...
<!-- <div class="sticky px-8 top-0"> -->
  <Navbar let:hidden let:toggle navClass="px-2 sm:px-4 py-2.5 absolute w-full z-20 top-0 left-0 border-b">
	...
	<div class="flex mr-4">
	  ...
	  <div class={`relative block ${$page.url.pathname !== '/movies' ? "hidden" : ""}`}>
		...
		<Input id="search-navbar" class="pl-10 pr-10" placeholder="Search..." bind:value={$searchQuery}/>

		<button on:click = {() => searchQuery.set("")} class={`flex absolute inset-y-0 right-0 items-center pr-3 ${$searchQuery == "" ? "hidden" : "block"}`}>
